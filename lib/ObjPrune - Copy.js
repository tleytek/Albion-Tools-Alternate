...
export const ObjPrune = (obj, enchantment) => {
  ...
  switch (enchantment) {
    case '@0':
      ...
      let finalObj = calculateEquipment(newObj)
...
const calculateEquipment = (fullRecipe) => {
  ...
  const fameArray = fullRecipe.craftingrequirements.craftresource.forEach(element => {
...
const calculateRefined = (refinedRecipe) => {
  ...
  for (let i = tier; i >= 2; i--){
    ...
    rawIndex = resources.Resource[_.findIndex(resources.Resource, { 'name': rawResource.uniquename.substring(3) })]
    ...
    if (i == 3) {
      currentRefinedRecipe = items.items[_.findIndex(items.items, {'uniquename': currentRefinedRecipe.craftingrequirements.craftresource[1].uniquename})]
    } else if (i >= 4) {
      currentRefinedRecipe = items.items[_.findIndex(items.items, {'uniquename': currentRefinedRecipe.craftingrequirements[0].craftresource[1].uniquename})]
    }
  }
